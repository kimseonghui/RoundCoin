...
const string strMessageMagic = "RoundCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // RoundCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // RoundCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    {
      nSubsidy = 10587000000 * COIN;
      return nSubsidy + nFees;
    }
    else if (pindexBest->nHeight+1 <= LAST_POW_BLOCK)
    {
      nSubsidy = 2.349981 * COIN;
      return nSubsidy + nFees;
    }
...
// RoundCoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // RoundCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // RoundCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // RoundCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // RoundCoin: track money supply and mint amount info
...
// RoundCoin: total coin age spent in transaction, in the unit of coin-days.
...
// RoundCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // RoundCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // RoundCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // RoundCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // RoundCoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // RoundCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // RoundCoin: check proof-of-stake
    ...
    // RoundCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // RoundCoin: getblocks may not obtain the ancestor block rejected
    ...
    // RoundCoin: if responsible for sync-checkpoint send it
...
// RoundCoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "RoundCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "2017-06-17T10:00:00+00:00";
        CTransaction txNew;
        txNew.nTime = 1497693600;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1497693600; //Friday, 24-Mar-17 02:44:04 UTC
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 6836272;
        if(fTestNet)
        {
            block.nNonce   = 6836272;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xa99d4945c5a5d4ac4057bf50b610840e335b58e4d04f5244500520793a382908"));
        block.print();
        ...
        // RoundCoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // RoundCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // RoundCoin: tell downloading node about the latest block if it's
