...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current RoundCoin address for receiving payments to this account.");
...
Value stakeforcharity(const Array &params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "stakeforcharity <RoundCoin address> <percent>\n"
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <RoundCoinaddress> <account>\n"
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <RoundCoinaddress> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <RoundCoinaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <RoundCoinaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <RoundCoinaddress> [minconf=1]\n"
            "Returns the total amount received by <RoundCoinaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <toRoundCoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid RoundCoin address: ")+s.name_);
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("RoundCoin-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("RoundCoin-lock-wa");
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // RoundCoin: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; RoundCoin server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <RoundCoinaddress>\n"
            "Return information about <RoundCoinaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <RoundCoinpubkey>\n"
            "Return information about <RoundCoinpubkey>.");
...
// RoundCoin: reserve balance from being staked for network protection
...
// RoundCoin: check wallet integrity
...
// RoundCoin: make a public-private key pair
...
Value cccustomchange(const Array& params, bool fHelp)
{
    ...
    if(!address.IsValid()) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid RoundCoin address");
...
Value ccsend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
		"ccsend <RoundCoin Address> <amount>\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value multisend(const Array &params, bool fHelp)
{
	...
	if (fHelp || params.size() != 2)
        throw runtime_error(
			...
			"multisend <RoundCoin Address> <percent>\n"
